...
def hypothesis_to_natural_language(df_current_pipeline):
...
def current_pipeline_to_information(df_current_pipeline, df_current_operator_results):
    return {
        'name': hypothesis_to_natural_language(df_current_pipeline),
        'size_output_set': df_current_pipeline['size_output_set'][0],
        ...
        'user_count': df_current_operator_results[df_current_operator_results['Region'] == 0]['cust_id'].nunique(),
        'movie_count': df_current_operator_results[df_current_operator_results['Region'] == 0]['article_id'].nunique(),
...
def current_operator_results_to_information(df_current_pipeline, df_current_operator_results):
    ...
    for region in df_current_operator_results['Region'].unique():
        ...
        if region == 0:
            ...
            current_result['name'] = get_htpothesis(df_current_pipeline['attributes_combination_output_data_regions'][0][region-1],
                                                    df_current_pipeline['output_data_regions'][0][region-1],
...
selected_value = 4
selected_next_data_region = None
...
def main(name='abc'):
    ...
    global current_operator_results
    
    global selected_region
    global selected_aggregation
    global selected_value
    global selected_next_data_region
    ...
    if len(current_pipeline) > 0:
        has_current_hypothesis = True
        current_hypothesis = current_pipeline_to_information(current_pipeline[-1], current_operator_results[-1])
        current_regions = current_operator_results_to_information(current_pipeline[-1], current_operator_results[-1])
    ...
    if request.method == 'POST':
        if request.form['action'] == 'start':
            ...
            reset = False
            next = False
        elif 'Unselecting' in request.form['action']:
            selected_region = 'Region0'
            selected_aggregation = None
            selected_value = 4
        elif 'Selecting' in request.form['action']:
            for i in range(1, current_operator_results[-1]['Region'].nunique()):
                selected_value = request.form.get('SelectRegion' + str(i))
                if selected_value is not None:
                    selected_next_data_region = current_pipeline[-1]['output_data_regions'][0][i-1]
                    selected_region = 'Region' + str(i)
                    if selected_value == '1':
                        selected_aggregation = ['One-Sample', 2.5, 'mean']
                    elif selected_value == '2':
                        selected_aggregation = ['One-Sample', 2.5, 'variance']
                    elif selected_value == '3':
                        selected_aggregation = ['One-Sample', 'uniform', 'distribution']
                    elif selected_value == '4':
                        selected_aggregation = None
    
    current_pipeline_information = []
    current_operator_results_information = []
    for element1, element2 in zip(list(reversed(current_pipeline[:-1])), list(reversed(current_operator_results[:-1]))):
        current_pipeline_information.append(current_pipeline_to_information(element1, element2))
        current_operator_results_information.append(current_operator_results_to_information(element1, element2))

    return render_template('index.html', start=start, next=next, has_current_hypothesis=has_current_hypothesis, current_hypothesis=current_hypothesis, current_regions=current_regions, current_pipeline=current_pipeline_information, current_operator_results=current_operator_results_information, selected_region=selected_region, selected_value=selected_value)
...
def job1():
    ...
    if start or next:
        if selected_aggregation is None:
            if (len(current_pipeline) == 0) or done:
            ...
            df = get_regions.get_results('power_only', prev_selected=selected_next_data_region, list_regions=None, agg_function=selected_aggregation[2])
            # df = pd.DataFrame(fakedata)
        ...
        if start or next:
            current_pipeline.append(df)
            current_operator_results.append(result1)
