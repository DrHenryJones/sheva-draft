...
def main():
    ...
    ###current_pipeline = get_regions.get_results(policy, prev_selected=selected_group, list_regions=list_regions, agg_function=aggregation)
    global fakedata
    current_pipeline = pd.DataFrame(fakedata)
    fakedata = fakedata2

    current_operator_result = getregions_hypo2(current_pipeline.iloc[0], originaldf_all)

    return jsonify({
        'pipeline': current_pipeline_to_information(current_pipeline, current_operator_result),
